...
def find_tracks(pdata, maxdist=20, giveup=10, n=0, stub=0,
    ...
    if n < 0 or stub > 0:
        ...
        if n < 0:
            track_lens_sort = np.argsort(track_lens)
            stubs = track_lens_sort[:n]  # all but the longest n
            print "\tkeeping {} longest tracks, lens {} to {}".format(
                -n, *track_lens[track_lens_sort[[n, -1]]])
        elif stub > 0:
    ...
    if n or stub > 0:
        if n > 0:
            ...
            keep_lens = track_lens[trackids[~isstub]]
            print "\tkeeping first {} tracks, lens {} to {}".format(
                n, keep_lens.min(), keep_lens.max())
            if early_stubs:
...
def plot_param(fits, param, fitx, fity, convert=None, ax=None, label='',
    ...
    if tag:
        tag = [t.replace('ree_lower_lid', '').replace('_50Hz_MRG', '')
               for t in tag]
        helplt.plt_text(valx, valy, tag, fontsize='x-small')
    return ax
...
if __name__ == '__main__':
    ...
    if args.check:
        ...
        helplt.animate(imstack, ftsets, fcsets, fosets, fisets, meta=meta,
    ...
    if args.plottracks and not args.check:
        ...
        helplt.plot_tracks(tracksets, bgimage, style='t',
